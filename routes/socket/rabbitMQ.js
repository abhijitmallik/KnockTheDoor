...
function RabbitMQ (server) {
    ...
    //return function () {
       ...
       // socketIo.socketId(server,path,config);
        ...
        // We will be overriding the emit function, so we'll make 
        // a copy of it for us to use later on
        var _emit = io.emit.bind(io);

        function processDataFromCached (message) {

            // If there is no value, or it can't be parsed to a
        ...
        // We will intercept the default behavior of the emit 
        // function. Not to worry, though. We are holding onto the 
        // real socket.io emit function and calling it _emit. We 
        // will call the _emit function when we check the cache 
        // and notice a change
        io.emit = function (topic, value) {
            ...
            _.each(config.server.port, function (portNumber) {
        ...
        amqp.connect(uri).then(function(connection) {
            console.log("===connection cretaed===",uri);
        ...
        console.log("9999999999999 rabbitMQ");
        socketIo.socketId(io);
