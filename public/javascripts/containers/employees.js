		...
		if(this.props.employees.employees.length > 0){
			...
			return(
				  <div className="emp-row" key={emp._id}>
				   {checkBox} <span className="emp-name">{emp.firstname} {emp.lastname}</span><span className="emp-occupation">{emp.occupation}</span><span className="emp-city">{emp.city}</span><span className="emp-state">{emp.state}</span><span className="emp-doj">{dt}</span><img className="emp-image"  src={emp.croppedImage}/><span className="delete-icon" title={titleMsg}></span>
				  </div>
				)
		    },this)
		    return(
